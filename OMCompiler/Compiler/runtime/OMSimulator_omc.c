...
void loadDLL()
{
  ...
  if(!OMSimulatorDLL)
  {
    ...
    //printf("%i path is:", strlen(path));
    char dllname[strlen(path)+25];
    sprintf(dllname,"%s%s%s%s", path,"/bin/","libOMSimulator",DLL_EXT);
    //printf("final path :");
    //printf("%s",dllname);
    #if defined WIN32
      OMSimulatorDLL = LoadLibrary(dllname);
    #elif defined(__APPLE__)
      OMSimulatorDLL = dlopen(dllname,RTLD_LAZY);
    #else
      OMSimulatorDLL = dlopen(dllname,RTLD_LAZY);
    #endif
    if(!OMSimulatorDLL)
